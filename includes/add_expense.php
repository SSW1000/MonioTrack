...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Retrieve form data
    $amount = filter_input(INPUT_POST, 'expenseAmount', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    $category = filter_input(INPUT_POST, 'category', FILTER_SANITIZE_STRING);
    $remarks = filter_input(INPUT_POST, 'remarks', FILTER_SANITIZE_STRING);
    $date = filter_input(INPUT_POST, 'expenseDate', FILTER_SANITIZE_STRING);
    $spent_using = filter_input(INPUT_POST, 'spentUsing', FILTER_SANITIZE_STRING);
    
    // Additional variable for bank_account_id
    $bank_account_id = null;

    if ($spent_using === 'Bank Balance') {
        // Retrieve the bank account ID from the form data
        $bank_account_id = filter_input(INPUT_POST, 'bankAccountE', FILTER_SANITIZE_NUMBER_INT);
        ...
        if ($bank_account_id !== null && $bank_account_id !== false) {
            ...
            $insert_query = "INSERT INTO expenses (amount, category, remarks, date, spent_using, user_id, bank_account_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $insert_stmt = mysqli_prepare($db, $insert_query);
            mysqli_stmt_bind_param($insert_stmt, "dssssii", $amount, $category, $remarks, $date, $spent_using, $user_id, $bank_account_id);
            ...
            if (mysqli_stmt_affected_rows($insert_stmt) > 0) {
                ...
                echo "Error adding expense record: " . mysqli_error($db); // Display SQL error message
            }
    ...
    } elseif ($spent_using === 'Cash') {
        // Perform the insertion without bank_account_id for Cash
        $insert_query = "INSERT INTO expenses (amount, category, remarks, date, spent_using, user_id) VALUES (?, ?, ?, ?, ?, ?)";
        $insert_stmt = mysqli_prepare($db, $insert_query);
        mysqli_stmt_bind_param($insert_stmt, "dssssi", $amount, $category, $remarks, $date, $spent_using, $user_id);
        ...
        echo "Error: Invalid 'spent_using' value.";
        exit;
    }
} else {
